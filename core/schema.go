...
// Including underscore preserves enum and custom type names (e.g. order_status) verbatim.
var dbTypeRe = regexp.MustCompile(`([a-zA-Z0-9_ ]+)(\((.+)\))?`)

// parseDBType parses the db type string into [baseType, typeArgs].
// Base type is preserved verbatim so enum types like "order_status" round-trip correctly.
